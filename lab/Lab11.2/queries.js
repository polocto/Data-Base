...
db.employees.find({},{"salary":0,"commision":0,"missions":0}).pretty();
...
db.employees.find({},{"_id":0,"name":1,"salary":1,"missions":{$slice:1}}).pretty();
// --

// the name and salary of the employees with a salary in the range [1,000; 2,500[
    db.employees.find({"salary":{$lt:2500,$gte:1000}},{"_id":0,"name":1,"salary":1}).pretty()
// the name and salary of the clerks with a salary in the range [1,000; 1,500[ (2 methods)
db.employees.find({"salary":{$lt:1500,$gte:1000},"job":"clerk"},{"_id":0,"name":1,"salary":1}).pretty()

// the employees whose manager is 7839 or whose salary is less than 1000
db.employees.find({"salary":{$lt:1000},"manager":7839}).pretty()
// the clerks and the analysts (2 methods)
db.employees.find({"job":{$in:["clerk","analyst"]}}).pretty()
...
db.employees.find({},{"_id":0,"name":1,"salary":1,"job":1}).sort({"job":1,"salary":-1}).pretty()
...
db.employees.find({"name":{$regex:"^S.*t$"}}).pretty()
// the employee names that contain a double 'l'
db.employees.find({"name":{$regex:"ll"}}).pretty()
// the employee names that begins with 'S' and contains either 'o' or 'm' (2 methods)
db.employees.find({"name":{$regex:"^S[om]"}}).pretty()
// --
...
db.employees.find({ "missions" : { $elemMatch : { "location" : "Chicago" } } } ).pretty()
// the employees who did a mission in Chicago or Dallas  (2 methods)
db.employees.find({ "missions" : { $elemMatch : { "location" : {$in:["Chicago","Dallas"]} } } } ).pretty()

// the employees who did a mission in Lyon and Paris (2 methods)
db.employees.find({ "missions" : { $elemMatch : { "location" : {$in:["Paris","Lyon"]} } } } ).pretty()
// the employees who did all their missions in Chicago
db.employees.find({ "missions.location" : { $all : [ "Chicago" ] } } ).pretty()
// the employees who did a mission for IBM in Chicago
db.employees.find({ "missions" : { $elemMatch : { "location" : "Chicago", "company":"IBM" } } } ).pretty()
